...
RgbColor backGround[3] = {RgbColor(25, 0, 25), RgbColor(0, 0, 50), RgbColor(50, 0, 0)};
uint16_t backGroundShift = 0;
...
void FadeOutAnimUpdate(const AnimationParam &param)
{
    ...
    if (param.progress == 1)
    {
        strip.SetPixelColor(animationState[param.index].IndexPixel, backGround[(animationState[param.index].IndexPixel + backGroundShift) % 3]);
    }
}
...
void LoopAnimUpdate(const AnimationParam &param)
{
    ...
    if (param.state == AnimationState_Completed)
    {
        ...
        if (frontPixel == 0)
        {
            ...
            backGroundShift = (backGroundShift + 1) % 3;
        }
...
void setup()
{
    ...
    // for (size_t i = 0; i < strip.PixelCount(); i += 3)
    // {
    //     strip.SetPixelColor(i, RgbColor(25, 0, 25));
    // }
    // for (size_t i = 1; i < strip.PixelCount(); i += 3)
    // {
    //     strip.SetPixelColor(i, RgbColor(0, 0, 50));
    // }
    // for (size_t i = 2; i < strip.PixelCount(); i += 3)
    // {
